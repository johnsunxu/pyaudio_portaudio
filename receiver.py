...
for device in outputDevices: 
    print(f"{outputDevices.index(device)}", device["name"], "Sample rate:", device["defaultSampleRate"])

#Get device info 
device_info = p.get_device_info_by_index(outputDevices[(int(input("Enter index of an output audio device above: ").strip()))]["index"])
...
while True: 
    try:
        ...
        def callback(outdata, frames, time, status):
            ...
            dataFromClient = clientConnected.recv(len(outdata[:]), socket.MSG_WAITALL)
        ...
        stream = sd.RawOutputStream(samplerate = outputRate, device = (device_info["index"]), callback=callback, finished_callback=event.set)
        with stream:
            event.wait()  
